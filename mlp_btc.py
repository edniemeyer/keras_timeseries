...
def evaluate_model(model, name, n_layers, ep, normalization, TRAIN_SIZE, dataset, ewm_btc, type):
    #X_train, X_test, Y_train, Y_test = dataset
    #X_trainp, X_testp, Y_trainp, Y_testp = dadosp
    if (normalization == 'AN'):
        X_train, X_test, Y_train, Y_test, scaler, shift_train, shift_test, X_trainp, X_testp, Y_trainp, Y_testp = nn_an_type(dataset, ewm_btc, TRAIN_SIZE, TARGET_TIME, LAG_SIZE, type)
    if (normalization == 'SW'):
    ...
    csv_logger = CSVLogger('output/%d_layers/btc_%s_%s.csv' % (n_layers, name, normalization))
    ...
    for o in range(2, 30, 30):
        for p in seed:
            ...
            for normalization in normalizations:
                ...
                trainScore, testScore, epochs, optimizer = evaluate_model(model, name, n_layers,nb_epoch, norm, TRAIN_SIZE, dataset, ewm_btc, type)
